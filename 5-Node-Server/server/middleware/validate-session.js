...
module.exports = function(req, res, next) {
    if (req.method == 'OPTIONS') {
        ...
        if (!sessionToken) 
            return res.status(403).send({ auth: false, message: 'No token provided.' }); //3
        else { //4
...
//2. The token is printed to the console. This is purely for debugging purposed to verify that the token is beng sent to the server. It should not be left in the final code, as it is a potential security vulnerability.
...
//5. The verify method decodes the token with the provided secret, then sends a callback with two variables. If successful, decoded will will contain the decoded payload; if not, decoded remains undefined. err is null by default.
...
//8. If no matchin id is found, an error message is thrown.
